...
export default React.memo(function ModalComponent({ setModalStatus }: PropsType) {
    ...
    const { register, handleSubmit, setValue, formState: { errors, touchedFields, isSubmitting, isDirty, isValid  }, watch } = useForm<ProfileInterface>({mode: 'onChange'})
    ...
    return (
        <div className={'modal'}>
            <form onSubmit={handleSubmit(onSubmit)}>
                <Input required={false} type={'text'} error={errors.name?.message} touched={touchedFields.name} serverError={nameError} register={register} name={'name'} patternValue={/^([a-zA-Z0-9]).{3,10}$/} minLength={3} maxLength={10} changeServerError={setNameError} placeholder={'Your new name'}/>
                <Input required={false} type={'text'} error={errors.location?.message} touched={touchedFields.location} register={register} name={'location'} patternValue={/^[a-zA-Z0-9]+$/} minLength={4} maxLength={15} changeServerError={setNameError} placeholder={'Your new location'}/>
                <div className={'files flex-between'}>
