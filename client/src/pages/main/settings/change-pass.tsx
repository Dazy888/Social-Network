...
const ChangePass = () => {
    ...
    const { mutateAsync, isLoading } = useMutation('change pass', (data: SetPassProps) => SettingsService.setPassword(data.currentPass, data.newPass, data.userId),
        {
            onSuccess() {
            ...
            <SettingsPage>
                ...
                <>
                    ...
                    <form className={'py-10 px-6'} onSubmit={handleSubmit(onSubmit)}>
                        <Input errorName={'password'} type={'password'} className={styles['big-input']} error={errors.currentPass?.message} touched={touchedFields.currentPass} serverError={passErr} register={register} name={'pass'} patternValue={passExp} minLength={4} maxLength={15} setServerError={setPassErr} placeholder={'Current password'}/>
